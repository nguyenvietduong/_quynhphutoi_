...
export type RelicForm = {
  ...
  description: string; era: string; worship: string; festival: string;
  ranking: string; recognizedYear: string;
  images: string[];
  verified: boolean; featured: boolean; active: boolean;
  seo?: SeoFields;
};

export const RELIC_FORM_EMPTY: RelicForm = {
  name: "", type: "",
  wardSlug: WARDS[0]?.slug ?? "", address: "",
  description: "", era: "", worship: "", festival: "",
  ranking: "", recognizedYear: "",
  images: [],
...
export function RelicEditorPage({
  editingSlug, initialForm, typeOptions, rankingOptions,
}: {
  editingSlug?: string;
  initialForm: RelicForm;
  typeOptions: CatOption[];
  rankingOptions: CatOption[];
}) {
  const router = useRouter();
  const { toast } = useToast();
  const [form, setForm] = useState<RelicForm>(initialForm);
  const [busy, setBusy] = useState(false);
  const [tab, setTab] = useState<Tab>("info");
  const [aiLoading, setAiLoading] = useState(false);
  const [aiProvider, setAiProvider] = useState<AiProvider>("gemini");

  function set<K extends keyof RelicForm>(k: K, v: RelicForm[K]) {
    setForm((f) => ({ ...f, [k]: v }));
  }

  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name.trim()) { toast.error("Nhập tên di tích."); setTab("info"); return; }
    if (!form.type) { toast.error("Chọn loại di tích."); setTab("info"); return; }
    ...
    try {
      const body = {
        ...
        description: form.description || undefined, era: form.era || undefined,
        worship: form.worship || undefined, festival: form.festival || undefined,
        ranking: form.ranking || undefined,
        recognizedYear: form.recognizedYear ? Number(form.recognizedYear) : undefined,
        images: form.images,
        verified: form.verified, featured: form.featured, active: form.active, seo: form.seo ?? {},
      };
      const res = await fetch(editingSlug ? `/api/admin/di-tich/${editingSlug}` : "/api/admin/di-tich",
      ...
      toast.success(editingSlug ? "Đã cập nhật di tích." : "Đã thêm di tích mới.");
  ...
  async function runAi() {
    ...
    setAiLoading(true);
    try {
      const res = await fetch("/api/admin/ai/di-tich", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: form.name, typeOptions, rankingOptions, wards: WARDS.map((w) => ({ slug: w.slug, name: w.name })), provider: aiProvider }),
      });
      ...
      setForm((f) => ({
        ...
        ranking:        data.rankingSlug  || f.ranking,
        wardSlug:       resolvedWardSlug  || f.wardSlug,
        address:        resolvedAddress   || f.address,
        era:            data.era          || f.era,
        worship:        data.worship      || f.worship,
        festival:       data.festival     || f.festival,
        recognizedYear: data.recognizedYear ? String(data.recognizedYear) : f.recognizedYear,
  ...
  const rankOpts: FilterOption[] = rankingOptions.map((o) => ({ value: o.slug, label: o.name }));

  return (
    <>
    <div className="qp-admin-head">
      <Link href="/admin/di-tich" className="qp-admin-head__eyebrow">Di tích</Link>
      <h1 className="type-h1">{editingSlug ? "Sửa di tích" : "Thêm di tích"}</h1>
    ...
    <form onSubmit={submit} className="qp-ae">
      <div className="qp-ae__topbar">
        ...
        <div className="qp-ae__headeractions">
          {editingSlug && (
            <a href={`/di-tich/${editingSlug}`} target="_blank" rel="noopener noreferrer" className="qp-btn-outline">Xem trang{SVG_EXT}</a>
          )}
          ...
          <Link href="/admin/di-tich" className="qp-btn-outline">Huỷ</Link>
          <button type="submit" className="qp-btn-primary" disabled={busy}>{busy ? "Đang lưu..." : editingSlug ? "Lưu thay đổi" : "Thêm di tích"}</button>
        </div>
      </div>
          ...
          <div className="qp-ae__tab-panel--detail">
            <div className="qp-ae__scard">
              ...
              <div className="qp-ae__scard-body">
                <div className="qp-ae__field">
                  <label className="qp-ae__flabel">Tên di tích <span className="req">*</span></label>
                  <input className="qp-input" value={form.name} maxLength={200} onChange={(e) => set("name", e.target.value)} placeholder="VD: Đình làng An Cầu" />
                </div>
                <div className="qp-ae__field">
                  <label className="qp-ae__flabel">Niên đại</label>
                  <input className="qp-input" value={form.era} maxLength={100} onChange={(e) => set("era", e.target.value)} placeholder="VD: Thời Trần (thế kỷ XIII)" />
                </div>
                <div className="qp-ae__field">
                  <label className="qp-ae__flabel">Thờ ai / Sự tích</label>
                  <input className="qp-input" value={form.worship} maxLength={200} onChange={(e) => set("worship", e.target.value)} placeholder="VD: Thờ Thành hoàng làng..." />
                </div>
                <div className="qp-ae__field">
                  <label className="qp-ae__flabel">Lễ hội chính</label>
                  <input className="qp-input" value={form.festival} maxLength={200} onChange={(e) => set("festival", e.target.value)} placeholder="VD: Hội làng ngày 15 tháng Giêng" />
                </div>
                <div className="qp-ae__field qp-ae__field--last">
                  <label className="qp-ae__flabel">Giới thiệu</label>
                  <textarea className="qp-textarea" rows={5} value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="Lịch sử, kiến trúc, giá trị văn hoá..." />
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
              <div className="qp-ae__scard">
                ...
                <div className="qp-ae__scard-body">
                  <div className="qp-ae__field">
                    <label className="qp-ae__flabel">Loại di tích <span className="req">*</span></label>
                    <FilterSelect options={typeOpts} value={form.type} onChange={(v) => set("type", v)} placeholder="Chọn loại" showSearch={false} />
                  </div>
                  <div className="qp-ae__field">
                    <label className="qp-ae__flabel">Xếp hạng</label>
                    <FilterSelect options={rankOpts} value={form.ranking} onChange={(v) => set("ranking", v)} placeholder="Chọn xếp hạng" showSearch={false} />
                  </div>
                  <div className="qp-ae__field qp-ae__field--last">
                    <label className="qp-ae__flabel">Năm được xếp hạng</label>
                    <input type="number" className="qp-input" value={form.recognizedYear} min={1945} max={2100} onChange={(e) => set("recognizedYear", e.target.value)} placeholder="VD: 1998" />
                  </div>
                </div>
              </div>
              <div className="qp-ae__scard">
        ...
        {tab === "address" && (
          <div className="qp-ae__tab-panel--detail">
            ...
            <div className="qp-ae__scard">
              ...
              <div className="qp-ae__scard-body">
                <ImageUploader max={5} value={form.images} onChange={(urls) => set("images", urls)} subfolder="di-tich" />
                <p className="qp-ae__cover-hint" style={{ marginTop: 8 }}>Tối đa 5 ảnh · Khuyến nghị tỉ lệ 4:3.</p>
              </div>
            </div>
          </div>
        )}

...
export type MarketForm = {
  ...
  description: string; schedule: string; priceText: string; unit: string;
  contactName: string; contactPhone: string;
  image: string; sourceUrl: string;
  verified: boolean; featured: boolean; active: boolean;
  seo?: SeoFields;
};

export const MARKET_FORM_EMPTY: MarketForm = {
  name: "", category: "",
  wardSlug: WARDS[0]?.slug ?? "", address: "",
  description: "", schedule: "", priceText: "", unit: "",
...
export function MarketEditorPage({
  editingSlug, initialForm, categoryOptions,
}: {
  editingSlug?: string;
  ...
  function set<K extends keyof MarketForm>(k: K, v: MarketForm[K]) {
    setForm((f) => ({ ...f, [k]: v }));
  }

  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name.trim()) { toast.error("Nhập tên mục."); setTab("info"); return; }
    if (!form.category) { toast.error("Chọn danh mục."); setTab("info"); return; }
    ...
    try {
      const body = {
        ...
        description: form.description || undefined, schedule: form.schedule || undefined,
        priceText: form.priceText || undefined, unit: form.unit || undefined,
        contactName: form.contactName || undefined, contactPhone: form.contactPhone || undefined,
        image: form.image || undefined, sourceUrl: form.sourceUrl || undefined,
        verified: form.verified, featured: form.featured, active: form.active, seo: form.seo ?? {},
      };
      const res = await fetch(editingSlug ? `/api/admin/cho/${editingSlug}` : "/api/admin/cho",
      ...
      toast.success(editingSlug ? "Đã cập nhật." : "Đã thêm mới.");
      router.push("/admin/cho");
  ...
  async function runAi() {
    ...
    setAiLoading(true);
    try {
      const res = await fetch("/api/admin/ai/cho", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: form.name, categoryOptions, wards: WARDS.map((w) => ({ slug: w.slug, name: w.name })), provider: aiProvider }),
      });
      ...
      setForm((f) => ({
        ...f,
        category:     data.categorySlug || f.category,
        wardSlug:     resolvedWardSlug  || f.wardSlug,
        address:      resolvedAddress   || f.address,
        description:  data.description  || f.description,
        schedule:     data.schedule     || f.schedule,
        priceText:    data.priceText    || f.priceText,
        seo: {
          ...f.seo,
  ...
  const catOpts: FilterOption[] = categoryOptions.map((o) => ({ value: o.slug, label: o.name }));

  return (
    <>
    <div className="qp-admin-head">
      <Link href="/admin/cho" className="qp-admin-head__eyebrow">Chợ &amp; Mua bán</Link>
      <h1 className="type-h1">{editingSlug ? "Sửa mục" : "Thêm mục mới"}</h1>
    ...
    <form onSubmit={submit} className="qp-ae">
      <div className="qp-ae__topbar">
        ...
        <div className="qp-ae__headeractions">
          {editingSlug && (
            <a href={`/cho/${editingSlug}`} target="_blank" rel="noopener noreferrer" className="qp-btn-outline">Xem trang{SVG_EXT}</a>
          )}
          ...
          <Link href="/admin/cho" className="qp-btn-outline">Huỷ</Link>
          <button type="submit" className="qp-btn-primary" disabled={busy}>{busy ? "Đang lưu..." : editingSlug ? "Lưu thay đổi" : "Thêm mục"}</button>
        </div>
      </div>
          ...
          <div className="qp-ae__tab-panel--detail">
            <div className="qp-ae__scard">
              ...
              <div className="qp-ae__scard-body">
                <div className="qp-ae__field">
                  <label className="qp-ae__flabel">Tên <span className="req">*</span></label>
                  <input className="qp-input" value={form.name} maxLength={200} onChange={(e) => set("name", e.target.value)} placeholder="VD: Chợ Quỳnh Côi, Bánh cuốn làng Kỳ Thư..." />
                </div>
                <div className="qp-acc-grid2">
                  <div className="qp-ae__field">
                    <label className="qp-ae__flabel">Giá tham khảo</label>
                    <input className="qp-input" value={form.priceText} maxLength={100} onChange={(e) => set("priceText", e.target.value)} placeholder="VD: 50.000đ/kg" />
                  </div>
                  <div className="qp-ae__field">
                    <label className="qp-ae__flabel">Đơn vị</label>
                    <input className="qp-input" value={form.unit} maxLength={50} onChange={(e) => set("unit", e.target.value)} placeholder="VD: kg, con, suất..." />
                  </div>
                </div>
                <div className="qp-ae__field">
                  <label className="qp-ae__flabel">Lịch họp / Giờ mở cửa</label>
                  <input className="qp-input" value={form.schedule} maxLength={200} onChange={(e) => set("schedule", e.target.value)} placeholder="VD: Phiên ngày 1,6 âm lịch / Hàng ngày 5:00–11:00" />
                </div>
                <div className="qp-acc-grid2">
                  <div className="qp-ae__field">
                    <label className="qp-ae__flabel">Tên liên hệ</label>
                    <input className="qp-input" value={form.contactName} maxLength={100} onChange={(e) => set("contactName", e.target.value)} />
                  </div>
                  <div className="qp-ae__field">
                    <label className="qp-ae__flabel">SĐT liên hệ</label>
                    <input className="qp-input" value={form.contactPhone} maxLength={20} onChange={(e) => set("contactPhone", e.target.value)} placeholder="0123 456 789" />
                  </div>
                </div>
                <div className="qp-ae__field">
                ...
                <div className="qp-ae__field qp-ae__field--last">
                  ...
                  <textarea className="qp-textarea" rows={5} value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="Giới thiệu, đặc điểm nổi bật..." />
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
              <div className="qp-ae__scard">
                ...
                <div className="qp-ae__scard-body">
                  <div className="qp-ae__field qp-ae__field--last">
                    <label className="qp-ae__flabel">Danh mục <span className="req">*</span></label>
                    <FilterSelect options={catOpts} value={form.category} onChange={(v) => set("category", v)} placeholder="Chọn danh mục" showSearch={false} />
                  </div>
                </div>
              </div>
              <div className="qp-ae__scard">
        ...
        {tab === "address" && (
          <div className="qp-ae__tab-panel--detail">
            ...
            <div className="qp-ae__scard">
              ...
              <div className="qp-ae__scard-body">
                <ImageUploader max={1} value={form.image ? [form.image] : []} onChange={(urls) => set("image", urls[0] ?? "")} subfolder="cho" />
                <p className="qp-ae__cover-hint" style={{ marginTop: 8 }}>Khuyến nghị tỉ lệ 4:3 · tối thiểu 800×600 px.</p>
              </div>
            </div>
          </div>
        )}
